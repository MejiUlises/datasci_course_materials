...
def main():
	...
	for token in texts:
		...
		for palabra in words:
			if palabra in scores:  #If the word is in the dictionary we grab the soore for the word
				sent_score += scores[palabra]
			else:  #if not, we add it to a dictionary with the score of the tweet
				new_words_sent[palabra] = 0

		for palabra in words:
			if palabra in new_words_sent:
				new_words_sent[palabra] += sent_score

		#print sent_score
		#sent_scores.append(sent_score)  #We append it to a list of scores

	for i in range(0, len(new_words_sent)):
		print new_words_sent.keys()[i], new_words_sent.values()[i]
